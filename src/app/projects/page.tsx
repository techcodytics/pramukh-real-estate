...
export default function ProjectsPage() {
  return (
    <div className="bg-white min-h-screen geometric-bg">
          ...
          {projectItems.map((project, index) => (
            ...
            >
              <Link href={`/projects/${project.id}`}>
                <div className="aspect-[16/10] bg-black overflow-hidden relative mb-10">
                  <img
                    src={project.image}
                    alt={project.title}
                    className="w-full h-full object-cover transition-all duration-1000 group-hover:scale-110 group-hover:brightness-50 grayscale hover:grayscale-0"
                  />
                  ...
                  <div className="absolute top-10 right-10 opacity-0 group-hover:opacity-100 transition-all duration-700 -translate-x-4 group-hover:translate-x-0">
                    <div className="w-16 h-16 rounded-full bg-white/90 backdrop-blur-md flex items-center justify-center text-black">
                ...
                <div className="flex justify-between items-end border-b border-black/5 pb-10">
                  <div>
                    <div className="flex items-center gap-3 text-black/40 font-sans font-bold text-[10px] uppercase tracking-[0.3em] mb-4">
                      <MapPin size={12} strokeWidth={2} /> {project.location}
                    </div>
                    <h3 className="text-4xl lg:text-5xl font-serif text-black mb-4 group-hover:text-black/60 transition-colors duration-500">
                      {project.title}
                    </h3>
                    <div className="flex items-center gap-6 text-black/20 font-sans text-sm font-light">
      ...
      <section className="bg-black py-32 px-6 lg:px-24 overflow-hidden relative">
        <div className="absolute top-0 right-0 w-128 h-128 bg-white/5 blur-[120px] rounded-full translate-x-1/2 -translate-y-1/2" />
        <div className="max-w-[1400px] mx-auto text-center">
          <h2 className="text-5xl md:text-7xl font-serif text-white mb-10">
            Your Sanctuary <br />{" "}
            <span className="italic text-white/40">is Waiting.</span>
          </h2>
          <Link
            href="/inquiry"
            className="serene-button inline-flex items-center gap-4 hover:bg-white! hover:text-black!"
          >
